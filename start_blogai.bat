...
cd /d "C:\Users\GKL\Downloads\blogai-final(2.0)"
...
if exist "%EDGE%" (
    start "" /b cmd /c "timeout /t 2 /nobreak > nul && start msedge http://localhost:3000"
) else if exist "%CHROME%" (
    start "" /b cmd /c "timeout /t 2 /nobreak > nul && start chrome http://localhost:3000"
) else (
